...
### A. 公開完了まで（これだけがサイト公開のブロッカー）

- [ ] **HTTPS 証明書の発行を確認**
      `gh api repos/zrockafropunx/afropunx-site/pages` の `https_certificate.state` が `issued`／`approved` になるか。
      DNS チェック通過（2026-07-19）から**最大 24 時間**。24h を大きく超える場合は Custom domain を Remove → 再設定で発行をやり直す
- [ ] **Enforce HTTPS にチェック**（Settings → Pages。証明書発行前は押せない）
- [ ] **Step 8 の動作確認**（https 表示 / http→https / www→apex リダイレクト / プレースホルダ 0 件）
