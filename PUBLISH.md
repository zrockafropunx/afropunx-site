...
| 6 | HTTPS の有効化 | ✅ 完了（2026-07-20） |
| 7 | WHOIS プライバシー | ✅ 元から有効・作業不要だった |
| 8 | 動作確認 | ✅ 完了（2026-07-20・全項目パス） |
| 9 | Play Console にウェブサイト登録 | ⬜ 未着手 |
| 追加 | `.nojekyll` / SPF / DKIM / DMARC | ✅ 完了 |

## ✅ サイト公開は完了（2026-07-20）

`https://afropunx.com` が有効な証明書付きで正常配信されている。**公開作業に残作業はない。**

```
証明書 : CN=afropunx.com / Let's Encrypt (YR2)
有効期間: 2026-07-19 23:15 GMT 〜 2026-10-17 23:15 GMT（自動更新）
SAN    : afropunx.com, www.afropunx.com
```

動作確認の実測結果（全項目パス）:

- `https://afropunx.com` → 200 OK・証明書検証 OK
- `http://afropunx.com` → `https://afropunx.com` へリダイレクト
- `https://www.afropunx.com` / `http://www.afropunx.com` → apex の https へリダイレクト
- 配信内容に プレースホルダ 0 件
...
- [x] **HTTPS 証明書の発行を確認** → `approved`
- [x] **Enforce HTTPS** → 有効
- [x] **Step 8 の動作確認** → 全項目パス

> **実測メモ（証明書が出ないときの対処）**: DNS チェック通過後、**約 24 時間経っても `https_certificate.state` が `null` のまま**という事象が発生した。
> CAA レコードの有無・A レコード 4 件・www の解決をすべて確認したが DNS 側に原因はなく、GitHub の発行処理が滞留していた。
> **Custom domain を Remove → 1 分待って再入力 → Save** で再スケジュールされ、直後に発行された。
> なお Remove すると GitHub がリポジトリの `CNAME` ファイルを自動削除し、再設定で作り直す（コミットが 2 つ増えるが無害）。

### B. メール認証の確認・運用（公開とは独立。設定自体は完了済み）

- [ ] **DKIM の実効性を確認**（**2026-07-21 以降に実施**）
      Google 側の反映に最大 48 時間かかるため、追加直後（2026-07-19）にテストしても PASS しない。
      support@afropunx.com から Gmail 宛てに送信 → 「メッセージのソースを表示」で
      `SPF: PASS` と `DKIM: 'PASS' with domain afropunx.com` を確認する
- [ ] **DMARC ポリシーの段階的引き上げ**（上記 DKIM の PASS 確認が前提・急がないこと）
      `p=none`（現在）→ `p=quarantine` → `p=reject` と数週間かけて上げる。
      **DKIM の PASS を確認しないまま引き上げると support@ が機能停止するため厳禁**

### C. Play 組織化に向けて（司令塔案件・本書の範囲外）

- [ ] **開業届の提出**（D-U-N-S 申請の前提。全体のクリティカルパスの起点）
- [ ] **掲載表記の確定とサイト反映**
      `開業届 → D-U-N-S → お支払いプロファイル → サイト` の順に上流が下流を縛る。
      未決: 住所を「1丁目4番3号」と「1-4-3」のどちらで統一するか / 電話 `05017844555` のハイフン有無。
      サイト現在値は `afropunx` / `神奈川県横浜市西区浅間町1丁目4番3号ウィザードビル402` / `05017844555`
- [ ] **D-U-N-S 申請**（東京商工リサーチ経由・発行まで最大 30 日）
      証憑については事前に問い合わせること。2025 年から税務署の紙提出に収受印が押されない運用のため、
      e-Tax の受信通知を証憑にするのが無難
- [ ] **Step 9: Play Console にウェブサイトを登録 → 確認リクエスト送信**（HTTPS 化後）

### D. 構造的リスク（要判断・期限なし）

- [ ] ドメイン更新の単一障害点への対処（決済失敗で**サイト・メール・事業者確認が同時全損**する）
- [ ] Google Workspace の特権管理者を複数化（現在 1 人のみ）
- [ ] GitHub リポジトリの Organization 化の検討（現在は個人アカウント。verified domain も個人紐付け）
